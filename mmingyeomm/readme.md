...
블록체인의 "블록"은 블록체인 기술의 핵심 요소로, 여러 트랜잭션(거래)의 데이터를 하나로 묶은 데이터 단위입니다. 
...
중앙화된 주체가 없는 P2P 네트워크 특성상 노드들이 각기 다른 네트워크의 상태를 주장하는 Fork가 일어날 수 있습니다. 비트코인의 같은 경우 동시에 다른 노드들이 채굴을 하게 된 경우, 혹은 네트워크 지연이나 통신 오류로 인해 Fork가 발생 할 수 있습니다.
...
비트코인의 합의 메커니즘에서 중요한 개념 중 하나는 **Longest Chain Rule(가장 긴 체인 규칙)**입니다. 이 규칙은 네트워크가 분기(fork)가 발생했을 때, **가장 긴 체인을 유효한 체인으로 간주**하는 원칙을 의미합니다.

##### 작동 방식:
1. 블록체인 네트워크에서 여러 채굴자가 동시에 블록을 생성하면, 체인이 분기될 수 있습니다.
2. 각 노드는 자신이 처음 받은 블록을 기준으로 체인을 계속 확장합니다.
3. 시간이 지나면서 한쪽 체인이 더 많은 블록을 추가하며 길어지게 됩니다.
4. 네트워크의 모든 노드는 **가장 긴 체인을 유효한 체인으로 선택**하고, 다른 체인의 블록은 폐기됩니다.

##### 주요 특징:
- **탈중앙화 보장**: 모든 노드가 동일한 체인을 따르도록 강제합니다.
- **확률적 파이널리티**: 블록이 체인에 추가된 후 시간이 지날수록 해당 블록이 폐기될 가능성이 낮아집니다.
- **공격 방지**: 공격자가 네트워크를 장악하려면 가장 긴 체인을 따라잡아야 하므로, 51% 이상의 해시파워가 필요합니다.

##### 예시:
- 채굴자 A와 B가 동시에 블록을 생성하여 체인이 두 갈래로 나뉘었다고 가정합니다.
- 이후 채굴자 C가 A의 체인 위에 새 블록을 추가하면, A의 체인이 더 길어집니다.
- 네트워크의 다른 노드들은 A의 체인을 선택하고, B의 체인은 폐기됩니다.
